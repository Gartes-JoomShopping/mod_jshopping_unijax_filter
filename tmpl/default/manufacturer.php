	...
	<?php if ($modHelper->params->show_manufacturers == 1 || $modHelper->params->show_manufacturers == 4) { ?>
		...
		<div class="uf_options_manufacturer<?php if ($modHelper->params->options_qnt && $modHelper->params->options_qnt < $count) { echo ' uf_overflow'; } ?><?php if ($modHelper->params->show_manufacturers == 1) { echo ' uf_hidecheckbox'; } ?>">
			...
			<div class="uf_input <?php echo $inputClass ?>">
                ...
                if ($modHelper->params->show_count_pos) { ?>
				<span class="uf_count">
					<?php
                ...
                if ($modHelper->params->show_manufacturers_link)
				{

				    ?>
				<a class="uf_link" href="<?= $rr =  \Joomla\CMS\Uri\Uri::root() . SEFLink('index.php?option=com_jshopping&controller=manufacturer&task=view&manufacturer_id=' . $v->id, 1) ?>" title="<?php echo JText::_('MOD_JSHOPPING_UNIJAXFILTER_MANUFACTURER_PRODUCTS').' '.htmlspecialchars($v->name) ?>"></a>
				<?php
                ...
                if ($modHelper->params->show_manufacturers_desc && $v->short_desc){
